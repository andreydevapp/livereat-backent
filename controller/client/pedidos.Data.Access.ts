...
import clienteModel  from '../../modelClient/userModel';
...
async function procesarPago(req:Request,res:Response){
    const valor = {colon:500, dolar:500}
    ...
    const accountBN = await gpApi.me.update({ bank_name: 'BN', test: false })
    console.log(accountBN);

    const customers = await gpApi.customers.fetch()
    console.log(customers);

    const totalEnDolar = valor.colon*valor.dolar;
    const pay = await gpApi.charges.create({
        amount: totalEnDolar,
...
async function loadDataToDb(req:Request,res:Response) {

    const negocio:any = req.body.negocio;
    console.log("negocio",negocio);
    let listaPlatillos:any = [];
    let totalSinIva:number = 0;
    let totalFinal:number = 0;
    let cobroIva:number = 0;
    let precioEnvio:number = 0;
    let iva = 0.13;
    let error = false;

    const negocioData:any = await negocioModel.findById(negocio.idNegocio);
    negocioDataGlobal = negocioData;
    const clienteData:any = await clienteModel.findById(req.body.idCliente);
    const cantiFacturas = await facturaModel.find().count();
    ...
    request.post('http://104.248.58.128:7777/firmar',{json:payloadFirm},
    ...
    if (negocioData && clienteData || negocioData.length > 0 && clienteData.length > 0) {
        ...
        for (const platillo of negocio.pedidos) {
            let platilloData:any = await platillosModel.findById(platillo.idPlatillo);
         ...
         const envioSolicitado:any = await negocio.zonasDeEnvio.filter(function(zona:any) {
         ...
         if (envioSolicitado.length > 0) {
             ...
             const zonaDeEnvio:any = await negocio.zonasDeEnvio.filter(function(zona:any) {
     ...
     if (!error) {
         ...
         await guardarFactura(req,res,clienteData,negocioData,listaPlatillos,totalSinIva,totalFinal,precioEnvio);
...
let facturaGlobal:any = [];

async function guardarFactura(req:Request, res:Response, cliente:any, negocio:any, platillos:any,totalSinIva:number,totalFinal:number,precioEnvio:number){
    ...
    const newFactura = new facturaModel(payloadFactura);
...
async function descargarImg(imgNegocio:string,req:Request, res:Response, negocioData:any){
    ...
    request({
        ...
        fs.writeFile(__dirname+'/../../temp/images/test.jpg', body, {
            encoding : null
        }, function(err) {
...
function crearPdf(req:Request, res:Response, negocioData:any){
    ...
    doc.pipe(fs.createWriteStream(__dirname+'/../../temp/pdf/'+ facturaGlobal._id +'.pdf'));
...
function generateCustomerInformation(doc:any) {
    ...
    doc
      ...
      .text(`Fecha: ${dateFormat(facturaGlobal.createAt, "dd-mm-yyyy h:MM TT")}`  , 50, 214)
      .text("Fecha vencimiento: 30/3/2020" , 50, 226)
      .text(`Condición de pago: ${facturaGlobal.condicionPago}`, 50, 238)
      .moveDown();
}

function generateInvoiceTable(doc:any) {
    doc
    .fontSize(14)
    ...
    for (i = 0; i < facturaGlobal.platillos.length; i++) {
      ...
      if (i+1 === facturaGlobal.platillos.length) {
        if (facturaGlobal.envio > 0) {
            ...
            generateTableRowEnvio(
                doc,
                position,
                "Tasa del servicio de envio del pedido",
                array,
                facturaGlobal.envio
            );    
...
function generateTableRow(doc:any, y:any, c1:any, array:any, c7:any) {
...
function generateTableRowEnvio(doc:any, y:any, c1:any,array:any , c7:any){
...
function generateTotal(doc:any,y:any){
...
function generateFooter(doc:any,y:any) {
    doc
      .fontSize(10)
      .font('Helvetica-Bold')
      .text(
      ...
      .fontSize(7)
      .text(
        "Comprobante electrónico emitido por Livereat",
        50,
        y+253,
        { align: "center", width: 500 }
      )
...
export async function obtenerFacturas(req:Request,res:Response){
    ...
    jwt.verify(token,'my_secret_token_Key',async (err:any,data:any)  =>{
        if (err) {
            ...
            const facturas:any = await facturaModel.find({"cliente.idCliente":req.body.idCliente}).sort({createAt: -1});
...
async function buildJsonToXml(res:Response, negocioData:any){
    ...
    request.post('http://104.248.58.128:7777/firmar',{json:payloadToFirm},
...
async function sendToXsd(res:Response,factura:any) {
    console.log("facturaxml",factura);

    request.post('http://localhost:3000/factura_electronica',{json:factura},
     (error:any, response:any, body:any) => {
      if (error) {
        console.error(error)
        return
      ...
      if (body.res === "Factura-procesada-correctamente") {
        ...
        var fs2 = require("fs");
        fs2.writeFile(__dirname+'/../../temp/xml/'+facturaGlobal._id+'.xml', xml, (err:any,data:any) => {
          if(err) {
            return console.log(err);
          }
          console.log("Archivo guardado");
          fs2.writeFile(__dirname+'/../../temp/xml/'+facturaGlobal._id+'.xml.xml', xmlHacienda, (err:any,data:any) => {
            if(err) {
              return console.log(err);
            }
            console.log("Archivo guardado");
...
async function sendEmail(res:Response) {
    ...
    transporter.sendMail({
        from: 'Andrey Castillo <aocastillo@est.utn.ac.cr>', // sender address
        to: emailCliente, // list of receivers
        subject:  `Factura Electrónica con número consecutivo ${facturaGlobal._id} del ${dateFormat(facturaGlobal.createAt, "dd-mm-yyyy h:MM TT")} emitida por ${facturaGlobal.negocio.nombreNegocio} para ${facturaGlobal.cliente.nombreUsuario}*`, // Subject line
        text: "Es un correo de prueba", // plain text body
        html:htmlClient, // html body
        attachments: [
            {
...
function sendEmailBusiness(){
    ...
    transporter.sendMail({
        from: 'Andrey Castillo <aocastillo@est.utn.ac.cr>', // sender address
        to: emailNegocio, // list of receivers
        subject:  `Factura Electrónica con número consecutivo ${facturaGlobal._id} del ${dateFormat(facturaGlobal.createAt, "dd-mm-yyyy h:MM TT")} emitida por ${facturaGlobal.negocio.nombreNegocio} para ${facturaGlobal.cliente.nombreUsuario}*`, // Subject line
        text: "", // plain text body
        html:htmlBusiness, // html body
        attachments: [
            {
                filename: facturaGlobal._id+'.pdf',
                path: __dirname+'/../../temp/pdf/'+ facturaGlobal._id +'.pdf'
